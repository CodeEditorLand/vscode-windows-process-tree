...
if (isWindows) {
	...
	describe("getRawProcessList", () => {
		...
		it("should return memory information only when the flag is set", (done) => {
			...
			native.getProcessList((list) => {
				assert.strictEqual(
					list?.every((p) => p.memory === undefined),
					true,
				);
		...
		it("should return command line information only when the flag is set", (done) => {
			...
			native.getProcessList((list) => {
				assert.strictEqual(
					list?.every((p) => p.commandLine === undefined),
					true,
				);
	...
	describe("getProcessList", () => {
		...
		it("should work promisified", async () => {
			const list: IProcessInfo[] = await promises.getProcessList(
				process.pid,
			);
				...
				() => {
					return new Promise((resolve) => {
						getProcessList(process.pid, (list) => {
							resolve(
								list?.length === 2 &&
									list![0].pid === process.pid &&
									list[1].pid === cps[0].pid,
							);
						});
					});
				},
	...
	describe("getProcessCpuUsage", () => {
		...
		it("should work promisified", async () => {
			const annotatedList: IProcessCpuInfo[] =
	...
	describe("getProcessTree", () => {
		...
		it("should work promisified", async () => {
			const tree: IProcessTreeNode = await promises.getProcessTree(
				process.pid,
			);
	...
	describe("contextAware", () => {
		it("should be context aware get process list", async () => {
			if (isMainThread) {
				const workerPromise: Promise<boolean> = new Promise(
					(resolve) => {
