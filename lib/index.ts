...
const native =
	process.platform === "win32"
		? require("../build/Release/windows_process_tree.node")
		: undefined;
import {
	IProcessInfo,
	IProcessTreeNode,
...
function buildRawTree(
	rootPid: number,
	processList: Iterable<IProcessInfo>,
): IProcessInfoNode | undefined {
...
export function buildProcessTree(
	...
	const buildNode = (
		{
			info: { pid, name, memory, commandLine },
			children,
		}: IProcessInfoNode,
		depth: number,
	): IProcessTreeNode => ({
...
export function filterProcessList(
	...
	function buildList(
		{ info, children }: IProcessInfoNode,
		depth: number,
		accum: IProcessInfo[],
	): IProcessInfo[] {
		accum.push(info);
...
export namespace getProcessList {
	...
	export const __promisify__ = (
		rootPid: number,
		flags?: ProcessDataFlag,
	): Promise<IProcessInfo[]> =>
		new Promise((resolve, reject) => {
			const callback = (processList: IProcessInfo[] | undefined) =>
				processList
					? resolve(processList)
...
export function getProcessCpuUsage(
	...
	callback: (tree: IProcessCpuInfo[]) => void,
): void {
	if (process.platform !== "win32") {
...
export namespace getProcessCpuUsage {
	...
	export const __promisify__ = (
		...
		new Promise((resolve, reject) => {
			...
			const callback = (cpuInfos: IProcessCpuInfo[] | undefined) =>
				cpuInfos
					? resolve(cpuInfos)
...
export namespace getProcessTree {
	...
	export const __promisify__ = (
		rootPid: number,
		flags?: ProcessDataFlag,
	): Promise<IProcessTreeNode> =>
		new Promise((resolve, reject) => {
			const callback = (tree: IProcessTreeNode | undefined) =>
				tree
					? resolve(tree)
					: reject(new Error(`Could not find PID ${rootPid}`));
...
[getProcessTree, getProcessList, getProcessCpuUsage].forEach((func) =>
	Object.defineProperty(func, promisify.custom, {
		enumerable: false,
		value: func.__promisify__,
	}),
);
